...
"""Tests for read_section handler and _collect_child_keys_for_node."""
...
def test_read_section_returns_rendered_content() -> None:
    ...
    tool = create_read_section_handler(
        ...
        param_lookup={PDTestParams: PDTestParams(name="hello")},
    )

    params = ReadSectionParams(section_key="summarized")
    result = tool.handler(params, context=make_pd_tool_context())  # type: ignore[arg-type]
...
def test_read_section_nested_key_parsing() -> None:
    ...
    parent = MarkdownSection[PDTestParams](
        ...
        children=[
            MarkdownSection[PDTestParams](
                ...
                visibility=SectionVisibility.SUMMARY,
                default_params=PDTestParams(name="child-value"),
            )
        ],
    )
    ...
    tool = create_read_section_handler(
        registry=snapshot,
        current_visibility=current_visibility,
        param_lookup={PDTestParams: PDTestParams(name="test")},
    )
...
def test_read_section_with_sibling_section() -> None:
    ...
    tool = create_read_section_handler(
        ...
        current_visibility=current_visibility,
        param_lookup={
            PDTestParams: PDTestParams(name="test"),
...
def test_read_section_with_nested_summarized_children() -> None:
    ...
    grandparent = MarkdownSection[PDTestParams](
        title="Grandparent",
