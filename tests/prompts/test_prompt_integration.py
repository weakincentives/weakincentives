...
def build_compose_prompt() -> PromptTemplate:
    ...
    return PromptTemplate.create(
        ns="tests.prompts", key="compose-email", sections=sections
    )
...
def test_prompt_integration_renders_expected_markdown() -> None:
    ...
    assert rendered.text == "\n\n".join(
        [
            "## 1. Message Routing (message-routing)\n\nTo: Jordan\nSubject: Q2 sync",
            "### 1.1. Tone (message-routing.tone)\n\nTarget tone: warm",
            "### 1.2. Content Guidance (message-routing.content-guidance)\n\nInclude the following summary:\nTop takeaways from yesterday's meeting.",
        ]
    )
