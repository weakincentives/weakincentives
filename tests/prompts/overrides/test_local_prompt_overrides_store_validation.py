...
"""Validation error tests for resolve, upsert, identifiers, headers, and loaders."""
...
def test_upsert_validation_errors(tmp_path: Path) -> None:
    ...
    section = descriptor.sections[0]
    tool = descriptor.tools[0]
    valid_sections = make_section_override(section)

    # Unknown section path
    override = make_override(
        descriptor, make_section_override(section, path=("unknown",))
    )
    with pytest.raises(PromptOverridesError):
        store.upsert(descriptor, override)
    ...
    # Wrong section hash
    override = make_override(
        descriptor, make_section_override(section, expected_hash=OTHER_DIGEST)
    )
    with pytest.raises(PromptOverridesError):
        store.upsert(descriptor, override)

    # Non-string body
    override = make_override(descriptor, make_section_override(section, body=123))
    with pytest.raises(PromptOverridesError):
        store.upsert(descriptor, override)
...
def test_resolve_header_validation_errors(tmp_path: Path) -> None:
    ...
    op.write_text(json.dumps(bad_version), encoding="utf-8")
    with pytest.raises(PromptOverridesError):
        store.resolve(descriptor)
    ...
    op.write_text(json.dumps(bad_metadata), encoding="utf-8")
    with pytest.raises(PromptOverridesError):
        store.resolve(descriptor)
