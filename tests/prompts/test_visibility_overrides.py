...
def test_cloned_session_preserves_visibility_reducers() -> None:
    ...
    assert len(cloned._registry.get_registrations(SetVisibilityOverride)) == 1
    assert len(cloned._registry.get_registrations(ClearVisibilityOverride)) == 1
    assert len(cloned._registry.get_registrations(ClearAllVisibilityOverrides)) == 1
