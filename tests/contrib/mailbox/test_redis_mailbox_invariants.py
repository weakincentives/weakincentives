...
class TestReceiptHandleFreshness:
    ...
    def test_redelivery_generates_new_handle(
        self, mailbox: RedisMailbox[Any, Any]
    ) -> None:
    ...
    def test_handle_unique_across_n_deliveries(
        ...
        mailbox: RedisMailbox[Any, Any] = RedisMailbox(
...
class TestMessageStateExclusivity:
    ...
    def test_receive_atomic_transition(
        self, mailbox: RedisMailbox[Any, Any], redis_client: Redis[bytes]
    ) -> None:
...
class TestNoMessageLoss:
    ...
    def test_crash_recovery_no_loss(self, redis_client: Redis[bytes]) -> None:
        ...
        mb1: RedisMailbox[Any, Any] = RedisMailbox(name=name, client=redis_client)
        for i in range(10):
            mb1.send(f"msg-{i}")
        mb1.close()

        # Second instance sees all messages
        mb2: RedisMailbox[Any, Any] = RedisMailbox(name=name, client=redis_client)
        try:
...
class TestDeliveryCountMonotonicity:
    ...
    def test_delivery_count_increments(self, mailbox: RedisMailbox[Any, Any]) -> None:
...
class TestVisibilityTimeout:
    ...
    def test_extend_prevents_requeue(self, mailbox: RedisMailbox[Any, Any]) -> None:
