...
"""Tests for host mount machinery and data types.

Moved with the machinery from ``tests/prompt/test_workspace_helpers.py``;
the assertions are unchanged and act as the parity oracle for the move.
"""
...
from weakincentives.sandbox._mounts import (
    _copy_mount_to_temp,
    _matches_globs,
    _resolve_mount_path,
)
...
class TestMaterializeMounts:
    def test_empty_mounts(self) -> None:
        path, previews = materialize_mounts([], allowed_host_roots=[])
        try:
            assert path.exists()
    ...
    def test_with_mounts(self, sample_source: Path) -> None:
        mount = HostMount(host_path=str(sample_source))
        path, previews = materialize_mounts(
    ...
    def test_custom_mount_path(self, sample_source: Path) -> None:
        ...
        path, _previews = materialize_mounts(
            [mount], allowed_host_roots=[sample_source.parent]
        )
        try:
    ...
    def test_custom_temp_dir_prefix(self, sample_source: Path) -> None:
        path, _previews = materialize_mounts(
            [], allowed_host_roots=[], temp_dir_prefix="wink-custom-"
        )
        try:
            assert path.name.startswith("wink-custom-")
        finally:
            shutil.rmtree(path, ignore_errors=True)
    ...
    def test_cleanup_on_error(self) -> None:
        ...
        with pytest.raises(FileNotFoundError):
            materialize_mounts([mount], allowed_host_roots=[])
...
class TestPathTraversal:
    def test_mount_path_traversal_rejected(self, sample_source: Path) -> None:
        ...
        with pytest.raises(WorkspaceSecurityError, match="escapes workspace"):
            materialize_mounts([mount], allowed_host_roots=[sample_source.parent])
