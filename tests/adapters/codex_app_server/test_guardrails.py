...
"""Tests for Codex App Server guardrails: feedback, task completion, policies."""
...
# ---- TestHandleToolCallFeedback ----


class TestHandleToolCallFeedback:
    """Tests for feedback collection in handle_tool_call."""
    ...
    def test_feedback_appended_on_success(self) -> None:
        ...
        async def _run() -> None:
            ...
            mock_prompt.feedback_providers = ()
            ...
            assert len(resp["contentItems"]) == 2
            assert resp["contentItems"][0]["text"] == "tool output"
            assert (
                resp["contentItems"][1]["text"] == "<feedback>test feedback</feedback>"
            )
    ...
    def test_no_feedback_when_none_returned(self) -> None:
        ...
        async def _run() -> None:
            ...
            resp = client.send_response.call_args[0][1]
            assert len(resp["contentItems"]) == 1

        asyncio.run(_run())

    def test_no_feedback_on_error_result(self) -> None:
        """Feedback is not collected when tool returns an error."""
