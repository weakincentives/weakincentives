...
"""Tests for TranscriptCollector core: discovery, tailing, lifecycle, errors."""
...
class TestTranscriptCollectorCore:
    """Tests for TranscriptCollector core functionality."""
    ...
    def test_main_transcript_discovery(
        ...
        result = asyncio.run(
            transcript_collector.hook_callback(
        ...
        assert transcript_collector._main_transcript_path == transcript_path
        assert transcript_collector._session_dir == transcript_temp_dir / "session123"
    ...
    def test_subagent_discovery(
        ...
        main_path = transcript_temp_dir / "session123.jsonl"
        main_path.touch()
        asyncio.run(transcript_collector._remember_transcript_path(str(main_path)))

        # Create subagent directory and transcripts
        subagents_dir = transcript_temp_dir / "session123" / "subagents"
        ...
        # Run discovery
        asyncio.run(transcript_collector._discover_subagents())

        # Should have discovered both subagents
        assert "subagent:001" in transcript_collector._tailers
        assert "subagent:002" in transcript_collector._tailers
        assert transcript_collector.subagent_count == 2
    ...
    def test_nonexistent_path_error_handling(
        ...
        fake_path = transcript_temp_dir / "nonexistent" / "session.jsonl"

        with patch(
            "weakincentives.adapters.claude_agent_sdk._transcript_collector.logger"
        ) as mock_logger:
            # Should not raise, just log warning
            asyncio.run(transcript_collector._remember_transcript_path(str(fake_path)))
    ...
    def test_hooks_config(self, transcript_collector: TranscriptCollector) -> None:
        """Hooks configuration returns correct structure."""
        hooks = transcript_collector.hooks_config()
    ...
    def test_context_manager_lifecycle(
        ...
        async def run_test() -> None:
            with patch("weakincentives.runtime.transcript._logger") as mock_logger:
                async with transcript_collector.run():
                    # Should be running
                    assert transcript_collector._running
                    assert transcript_collector._poll_task is not None
                    assert transcript_collector._discovery_task is not None

                    # Wait a bit for polling
                    await asyncio.sleep(0.05)

                # Should have stopped
                assert not transcript_collector._running
    ...
    def test_multiple_transcripts_parallel(
        ...
        assert transcript_collector.subagent_count == 2
        assert transcript_collector.total_entries == 3
        assert len(transcript_collector.transcript_paths) == 3
    ...
    def test_hook_callback_without_transcript_path(
        ...
        assert result == {}
        assert transcript_collector._main_transcript_path is None

    def test_poll_loop_exits_when_not_running(
        self, transcript_collector: TranscriptCollector
    ) -> None:
        """Polling loop should exit immediately when _running is False."""
        assert transcript_collector._running is False
        asyncio.run(transcript_collector._poll_loop())
    ...
    def test_context_manager_handles_missing_task_refs(
        ...
        async def run_test() -> None:
            async with transcript_collector.run():
                poll_task = transcript_collector._poll_task
                discovery_task = transcript_collector._discovery_task
                assert poll_task is not None
                assert discovery_task is not None

                # Simulate losing references to the tasks before exit.
                transcript_collector._poll_task = None
                transcript_collector._discovery_task = None
