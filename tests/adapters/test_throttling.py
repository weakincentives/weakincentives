...
def test_runner_retries_after_throttle(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("weakincentives.adapters.throttle.sleep_for", _sleep)
    monkeypatch.setattr(
...
def test_runner_bubbles_throttle_when_budget_exhausted(
    ...
    monkeypatch.setattr(
        "weakincentives.adapters.throttle.sleep_for", lambda _delay: None
    )
...
def test_runner_deadline_prevents_retry(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "weakincentives.adapters.throttle.jittered_backoff",
