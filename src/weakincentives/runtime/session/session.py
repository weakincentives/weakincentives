...
class Session(SessionProtocol):
    ...
    def __init__(
        ...
        *,
        ...
        # Shared lock for all managers
        self._lock = RLock()

        # Composed managers
        self._state_store = StateStore(self._lock)
        self._observer_manager = ObserverManager(self._lock)
        self._snapshot_manager = SnapshotManager(self._lock, self._state_store)

        # Initialize default slice policies for built-in event types
        self._state_store.set_initial_policies(
            {
    ...
    def clone(
        ...
        *,
        ...
        reducer_snapshot, state_snapshot, policy_snapshot = (
            self._state_store.snapshot_for_clone()
        )
        clone = Session(
            bus=bus,
        ...
        clone._state_store.copy_reducers_from_snapshot(reducer_snapshot)
        clone._state_store.apply_state_snapshot(state_snapshot)
        clone._state_store.apply_policy_snapshot(policy_snapshot)
        return clone

    # ──────────────────────────────────────────────────────────────────────
    # SliceAccessor Integration (used by SliceAccessor)
    # ──────────────────────────────────────────────────────────────────────
    ...
    def _select_all[S: SupportsDataclass](self, slice_type: type[S]) -> tuple[S, ...]:
        ...
        for public access.
        """
        return self._state_store.select_all(slice_type)
    ...
    def observe[S: SupportsDataclass](
        ...
        """
        return self._observer_manager.observe(slice_type, observer)
    ...
    def restore(
        ...
        """
        self._snapshot_manager.restore(snapshot, preserve_logs=preserve_logs)
    ...
    def _mutation_seed_slice[S: SupportsDataclass](
        ...
        self._state_store.seed_slice(slice_type, values)
    ...
    def _mutation_clear_slice[S: SupportsDataclass](
        ...
        self._state_store.clear_slice(slice_type, predicate)
    ...
    def _mutation_register_reducer[S: SupportsDataclass](
        ...
        *,
        ...
        self._state_store.register_reducer(
            data_type, reducer, slice_type=slice_type, policy=policy
        )
    ...
    def snapshot(
        ...
        *,
        ...
        return self._snapshot_manager.capture(
    ...
    def _dispatch_data_event(
        ...
        state_changes = self._state_store.dispatch_event(
            data_type, event, context=context
        )

        # Notify observers of state changes
        self._observer_manager.notify_observers(state_changes)
    ...
    def _register_builtin_reducers(self) -> None:
        ...
        if self._state_store.has_reducer_for(SetVisibilityOverride):
            return
