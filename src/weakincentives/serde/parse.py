...
def _coerce_literal_value(value: object, literal: object) -> object:
    if isinstance(literal, bool) and isinstance(value, str):
        return _bool_from_str(value)

    literal_constructor = cast(Callable[[object], object], type(literal))
    return literal_constructor(value)
...
def _coerce_literal(
    ...
    for literal in literals:
        ...
        if config.coerce:
            try:
                coerced_literal = _coerce_literal_value(value, literal)
            except (TypeError, ValueError) as error:
...
def _coerce_dataclass(
    ...
    if not (isinstance(base_type, type) and dataclasses.is_dataclass(base_type)):
        return _NOT_HANDLED
    dataclass_type = cast(DataclassType, base_type)
...
def _coerce_to_type(
    ...
    if callable(base_type):
        callable_type = cast(Callable[[object], object], base_type)
        try:
            coerced = callable_type(value)
        except Exception as error:
            raise type(error)(str(error)) from error
        return apply_constraints(coerced, merged_meta, path)

    raise TypeError(f"{path}: unable to coerce value using {base_type!r}")
