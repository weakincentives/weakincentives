...
class PodmanSandboxSection(MarkdownSection[_PodmanSectionParams]):
    ...
    def __init__(
        ...
        *,
        ...
        ) = resolve_podman_host_mounts(self._mounts, self._allowed_roots)
        if _overlay_path is not None and _filesystem is not None:
            ...
            for mount in self._resolved_mounts:
                copy_mount_into_overlay(overlay=self._overlay_path, mount=mount)
        ...
        self._exec_runner = config.exec_runner or _default_exec_runner
        ...
        self._shell_suite = PodmanShellSuite(section=self)
    ...
    def _ensure_workspace(self) -> WorkspaceHandle:
        with self._lock:
    ...
    def _create_workspace(self) -> WorkspaceHandle:
    ...
    def _hydrate_overlay_mounts(self, overlay: Path) -> None:
        ...
        for mount in self._resolved_mounts:  # pragma: no cover
            copy_mount_into_overlay(overlay=overlay, mount=mount)  # pragma: no cover
    ...
    def _touch_workspace(self) -> None:
        with self._lock:
            ...
            self._workspace_handle = WorkspaceHandle(
    ...
    def ensure_workspace(self) -> WorkspaceHandle:
    ...
    def write_via_container(
        ...
        *,
        ...
        from .podman_lifecycle import container_path_for

        handle = self.ensure_workspace()
        host_path = host_path_for(handle.overlay_path, path)
        payload = content
        ...
        container_path = container_path_for(path)
        parent = posixpath.dirname(container_path)
        if parent and parent != "/":
            mkdir_result = self.run_cli_exec(
                config=ShellExecConfig(command=("mkdir", "-p", parent), cwd="/"),
            )
    ...
    def new_client(self) -> PodmanClient:
    ...
    @property
    def exec_runner(self) -> Callable[..., subprocess.CompletedProcess[str]]:
