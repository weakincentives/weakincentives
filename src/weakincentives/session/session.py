...
class Session:
    ...
    def clone(
        ...
        *,
        ...
        with self._lock:
            reducer_snapshot = {
                data_type: list(registrations)
                for data_type, registrations in self._reducers.items()
            }
            state_snapshot = dict(self._state)
            current_bus = self._bus

        clone._reducers = {
            data_type: list(registrations)
            for data_type, registrations in reducer_snapshot.items()
        }
        clone._state = state_snapshot

        clone._bus = current_bus if bus is None else bus
        if bus is not None:
    ...
    def snapshot(self) -> Snapshot:
        ...
        try:
            normalized: Mapping[type[Any], tuple[Any, ...]] = normalize_snapshot_state(
                cast(Mapping[object, tuple[object, ...]], current_state)
            )
    ...
    def _registered_slice_types(self) -> set[type[Any]]:
        with self._lock:
            return self._registered_slice_types_unlocked()

    def _registered_slice_types_unlocked(self) -> set[type[Any]]:
    ...
    def _dispatch_data_event(
        ...
        with self._lock:
            ...
            if not registrations:
                ...
                self._reducers[data_type] = registrations
                for registration in registrations:
                    _ = self._state.setdefault(registration.slice_type, ())
