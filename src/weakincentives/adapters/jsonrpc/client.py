...
"""Bidirectional JSON-RPC client for agent servers.

Supports three transport modes:

- **stdio** (default): spawns an agent binary as a subprocess and
  communicates via newline-delimited JSON over stdin/stdout pipes.
- **managed WebSocket**: spawns the binary with a ``--listen ws://…``
  flag and connects to it via WebSocket.
- **external WebSocket**: connects to an already-running server at a
  ``ws://`` or ``wss://`` URL.  No subprocess is spawned or managed.
...
logger: StructuredLogger = get_logger(__name__, context={"component": "jsonrpc_client"})
...
class JsonRpcClientError(Exception):
    """Error from the JSON-RPC client."""
...
class JsonRpcClient:
    ...
    *transport* selects the wire protocol (``"stdio"`` or ``"websocket"``).
    ...
    subprocess with managed-WS args and connects to it.
    """
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        self._bin_args = bin_args
        self._bin_ws_args = bin_ws_args
    ...
    async def send_request(
        ...
        :class:`JsonRpcClientError` if the response contains an ``error``
        field.
        """
        self._next_id += 1
        ...
        if read_task is None:
            raise JsonRpcClientError("Client not started")
        if read_task.done():
            raise JsonRpcClientError("Transport disconnected unexpectedly")
        ...
        try:
            ...
            if isinstance(exc, JsonRpcClientError):
                raise
            raise JsonRpcClientError(
                f"Failed to send request {method} (id={req_id})"
            ) from exc

        # If read loop exited between pending registration and write
        # completion, fail fast instead of waiting forever when timeout=None.
        if read_task.done() and not future.done():
            _ = self._pending.pop(req_id, None)
            future.set_exception(
                JsonRpcClientError("Transport disconnected unexpectedly")
            )
        ...
        if "error" in resp:
            raise JsonRpcClientError(f"{method} failed: {resp['error']}")
    ...
    async def _start_stdio(self) -> None:
        """Spawn the agent subprocess and begin reading."""
        self._proc = await self._spawn(*self._bin_args, stdin=asyncio.subprocess.PIPE)
        self._read_task = asyncio.create_task(self._read_loop())
        self._start_stderr_capture()

    async def _start_ws_managed(self) -> None:
        """Spawn agent with ``--listen ws://…`` and connect.

        Retries with a fresh port if the selected port is already taken
        (TOCTOU race between ``_find_free_port`` and subprocess bind).
        """
        last_error: JsonRpcClientError | None = None
        ...
        if last_error is not None:  # pragma: no branch — loop runs at least once
            raise last_error
        raise JsonRpcClientError(  # pragma: no cover
            "Managed WebSocket startup failed"
        )

    async def _start_ws_external(self) -> None:
        """Connect to an external server via WebSocket."""
        url = self._remote_url
        if url is None:  # pragma: no cover — guarded by start()
            raise JsonRpcClientError("remote_url is required for external WS mode")
    ...
    async def _spawn(
        ...
        *args: str,
        ...
        """Spawn the agent binary as a subprocess."""
    ...
    async def _await_tcp_ready(self, host: str, port: int, label: str) -> None:
        ...
        for _ in range(_WS_CONNECT_MAX_RETRIES):
            if self._proc is not None and self._proc.returncode is not None:
                raise JsonRpcClientError(
                    f"Agent exited during startup (code={self._proc.returncode}): {self.stderr_output[-2000:]}"
                )
            try:
        ...
        raise JsonRpcClientError(
            f"Agent did not start listening on {label} within {_WS_CONNECT_MAX_RETRIES * _WS_CONNECT_RETRY_DELAY}s"
        )
    ...
    async def _connect_ws(self, url: str) -> None:
        ...
        try:
            ...
            msg = (
                ...
                " — install via: pip install weakincentives[codex-ws]"
            )
            raise JsonRpcClientError(msg) from exc
        ...
        try:
            ...
            raise JsonRpcClientError(f"Failed to connect to {url}: {exc}") from exc
    ...
    async def _write(self, msg: dict[str, Any]) -> None:
        ...
        if self._ws is not None:
            try:
                ...
                raise JsonRpcClientError(f"WebSocket send failed: {exc}") from exc
        elif self._proc is not None and self._proc.stdin is not None:
            self._proc.stdin.write((data + "\n").encode())
            await self._proc.stdin.drain()
        else:
            raise JsonRpcClientError("Client not started")  # pragma: no cover
    ...
    async def _read_loop(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "jsonrpc_client.read_loop_error",
    ...
    def _fail_pending(self, message: str) -> None:
        ...
        for future in self._pending.values():  # pragma: no branch
            if not future.done():  # pragma: no branch
                future.set_exception(JsonRpcClientError(message))
        self._pending.clear()
