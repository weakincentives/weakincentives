...
"""Guardrails support for the ACP adapter.
...
def append_feedback(
    ...
    *,
    ...
    if feedback_text:
        content_items.append({"type": "text", "text": feedback_text})
...
def check_task_completion(  # noqa: PLR0911
    *,
    ...
    if deadline is not None and deadline.remaining().total_seconds() <= 0:
        logger.debug(
            "acp.task_completion.deadline_exhausted",
    ...
    if budget_tracker is not None:
        ...
        try:
            ...
            logger.debug(
                "acp.task_completion.budget_exhausted",
    ...
    if not result.feedback:
        logger.debug(
            "acp.task_completion.incomplete_no_feedback",
            event="task_completion.incomplete_no_feedback",
        )
        return False, None

    logger.info(
        "acp.task_completion.incomplete",
