...
from ..runtime.session import Session, WorkspaceDigest
from ._types import SupportsDataclass
from .section import Section

_LOGGER: StructuredLogger = get_logger(
    __name__, context={"component": "prompt.workspace_digest"}
)
...
class WorkspaceDigestSection(Section[SupportsDataclass]):
    ...
    def _resolve_body(self, override_body: str | None = None) -> str:
        digest = cast(
            WorkspaceDigest | None, self._session.workspace_digest.latest(self.key)
        )
...
__all__ = ["WorkspaceDigestSection"]
