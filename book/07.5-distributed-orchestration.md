...
Dead Letter Queues (DLQs) capture messages that cannot be processed after repeated attempts. This prevents poison messages from blocking queues while preserving them for inspection and remediation.

**Use DLQ for:**

- Poison message isolation (permanently failing messages)
- Failure forensics (inspect what went wrong)
- Manual remediation (fix data and resubmit)
- Retry orchestration (separate queue for manual retries)

**Not for:**

- Transient failures (use visibility backoff instead)
- Rate limiting (use throttling instead)
- Validation errors (reject immediately, don't retry)

#### DLQPolicy Configuration

`DLQPolicy` combines a destination mailbox with decision logic for when to dead-letter failed messages:
...
# Create DLQ mailbox
dead_letters = RedisMailbox[DeadLetter[MainLoopRequest], None](
    name="dead-letters",
    client=redis_client,
)
...
dlq_policy = DLQPolicy(
    ...
    max_delivery_count=5,  # Dead-letter after 5 failures
)
```

**DLQPolicy parameters:**

| Parameter | Default | Purpose |
|-----------|---------|---------|
| `mailbox` | (required) | Destination mailbox for dead-lettered messages |
| `max_delivery_count` | `5` | Maximum delivery attempts before dead-lettering |
| `include_errors` | `None` | Exception types that trigger immediate dead-lettering |
| `exclude_errors` | `None` | Exception types that never dead-letter (always retry) |

#### MainLoop Integration

Pass `DLQPolicy` to `MainLoop` constructor:

```python
from weakincentives.runtime import MainLoop, DLQPolicy

class MyMainLoop(MainLoop[UserRequest, Output]):
    def prepare(self, request: UserRequest) -> tuple[Prompt, Session]:
        # ... create prompt and session
        pass

    def finalize(self, prompt: Prompt, session: Session) -> None:
        # ... cleanup
        pass

# Create loop with DLQ
loop = MyMainLoop(
    adapter=adapter,
    requests=requests,
    dlq=DLQPolicy(
...
2. Calls `dlq.should_dead_letter(msg, error)`
3. If `True`: Sends `DeadLetter` envelope to DLQ and acknowledges
4. If `False`: Nacks with exponential backoff for retry

#### DeadLetter Envelope

Dead-lettered messages are wrapped in a `DeadLetter` envelope with failure metadata:
...
class DeadLetter[T]:
    ...
    body: T                      # Original message body
    source_mailbox: str          # Name of source mailbox
    delivery_count: int          # Attempts before dead-lettering
    last_error: str              # String representation of error
    last_error_type: str         # Fully qualified error type name
    dead_lettered_at: datetime   # When dead-lettered
    first_received_at: datetime  # First delivery timestamp
    request_id: UUID | None      # Request ID (if MainLoopRequest/EvalRequest)
    reply_to: str | None         # Original reply-to mailbox
    trace_id: str | None         # Distributed tracing ID
```

This envelope preserves all context needed for debugging and remediation.

#### Custom Dead-Letter Logic

Subclass `DLQPolicy` to customize dead-letter behavior:

```python
from weakincentives.runtime import DLQPolicy
from my_app.errors import ValidationError, AuthenticationError

class CustomDLQPolicy(DLQPolicy[MainLoopRequest, MainLoopResult]):
    """Custom DLQ logic with error-specific handling."""

    def should_dead_letter(self, message: Message, error: Exception) -> bool:
        """Custom dead-letter decision logic."""

        # Validation errors: dead-letter immediately (won't succeed on retry)
        if isinstance(error, ValidationError):
            return True

        # Auth errors: dead-letter immediately (credentials issue)
        if isinstance(error, AuthenticationError):
            return True

        # Transient network errors: never dead-letter (keep retrying)
        if isinstance(error, (ConnectionError, TimeoutError)):
            return False

        # Everything else: use delivery count threshold
        return message.delivery_count >= self.max_delivery_count
```

**Using `include_errors` and `exclude_errors`:**

For simple error filtering, use the built-in parameters instead of subclassing:
...
dlq_policy = DLQPolicy(
    mailbox=dead_letters,
    max_delivery_count=5,
    # Dead-letter these immediately (no retries)
    include_errors=frozenset({ValidationError, AuthenticationError}),
    # Never dead-letter these (infinite retries)
    exclude_errors=frozenset({ConnectionError, TimeoutError}),
)
```

#### Example: Production Code Review Loop

```python
from weakincentives.runtime import MainLoop, DLQPolicy, DeadLetter
from weakincentives.runtime.mailbox import RedisMailbox

# Dead-letter mailbox for failed reviews
dead_letters = RedisMailbox[DeadLetter[PRReviewRequest], None](
    name="pr-reviews-dlq",
    client=redis_client,
)

# DLQ policy: 5 retries, immediate dead-letter on validation errors
class ReviewDLQPolicy(DLQPolicy[PRReviewRequest, ReviewReport]):
    def should_dead_letter(self, message: Message, error: Exception) -> bool:
        # Invalid PR URL: dead-letter immediately
        if isinstance(error, InvalidPRError):
            return True

        # Repository not found: dead-letter immediately
        if isinstance(error, RepositoryNotFoundError):
            return True

        # LLM rate limit: never dead-letter (transient)
        if isinstance(error, RateLimitError):
            return False

        # Default: delivery count threshold
        return message.delivery_count >= self.max_delivery_count

# Code review loop with DLQ
class CodeReviewLoop(MainLoop[PRReviewRequest, ReviewReport]):
    # ... prepare/finalize methods ...
    pass

loop = CodeReviewLoop(
    adapter=ClaudeAgentSDKAdapter(),
    requests=RedisMailbox(name="pr-reviews", client=redis_client),
    dlq=ReviewDLQPolicy(
        mailbox=dead_letters,
        max_delivery_count=5,
    ),
...
When a PR review fails:

- **Invalid PR URL**: Dead-lettered immediately (won't fix itself)
- **Repository not found**: Dead-lettered immediately (permanent issue)
- **Rate limit error**: Retried indefinitely with backoff (transient)
- **Other errors**: Retried up to 5 times, then dead-lettered

#### Monitoring and Remediation

Process dead-lettered messages for inspection and remediation:

```python
# DLQ consumer for monitoring
dead_letter_consumer = RedisMailbox[DeadLetter[PRReviewRequest], None](
    name="pr-reviews-dlq",
    client=redis_client,
)

for msg in dead_letter_consumer.receive():
    dead_letter = msg.body
    ...
    # Acknowledge (message is logged)
    msg.acknowledge()
```

**Remediation workflow:**

1. **Inspect dead letters**: Review error details and request context
2. **Fix root cause**: Update code, fix data, adjust configuration
3. **Resubmit**: Send corrected requests back to original queue
4. **Monitor**: Track DLQ rates to identify systemic issues

```python
# Manual remediation: resubmit after fixing data
def resubmit_fixed_request(dead_letter: DeadLetter[PRReviewRequest]) -> None:
    """Fix and resubmit a dead-lettered PR review request."""

    # Fix the issue (e.g., correct invalid PR URL)
    fixed_request = fix_pr_url(dead_letter.body)

    # Resubmit to original queue
    original_queue = RedisMailbox[PRReviewRequest, ReviewReport](
        name=dead_letter.source_mailbox,
        client=redis_client,
    )

    original_queue.send(fixed_request)
    logger.info(f"Resubmitted fixed request {dead_letter.request_id}")
```

#### EvalLoop DLQ Support

`EvalLoop` includes identical DLQ support for handling failing evaluation samples:

```python
from weakincentives.runtime import EvalLoop, DLQPolicy

eval_dlq = DLQPolicy(
    mailbox=eval_dead_letters,
    max_delivery_count=3,  # Fewer retries for evaluations
)
...
1. **Always configure DLQ in production**: Prevents poison messages from blocking queues
2. **Set appropriate retry counts**: Balance between retry tolerance and fast failure (typically 3-5)
3. **Use `include_errors` for permanent failures**: Validation errors, authentication errors
4. **Use `exclude_errors` for transient failures**: Network errors, rate limits
5. **Monitor DLQ rates**: High DLQ rates indicate systemic issues
6. **Implement remediation workflows**: Don't let dead letters accumulate indefinitely
7. **Include trace IDs**: Use `trace_id` for distributed tracing correlation
8. **Log dead-letter events**: Capture full context for forensics

#### Comparison: Retry vs. Dead-Letter

| Scenario | Retry | Dead-Letter |
|----------|-------|-------------|
| Transient network error | ✅ Retry with backoff | ❌ |
| Rate limit exceeded | ✅ Retry after cooldown | ❌ |
| Invalid input data | ❌ | ✅ Immediate DLQ |
| Missing credentials | ❌ | ✅ Immediate DLQ |
| LLM timeout (transient) | ✅ Retry | ❌ |
| Code bug (permanent) | ❌ After N retries | ✅ DLQ |
...
For long-running agent workflows, manually extending visibility timeout is error-prone and doesn't handle stuck workers correctly. WINK provides **LeaseExtender**: a heartbeat-driven automatic extension mechanism that ties lease renewal to proof-of-work.

**The problem with manual extension:**

```python
# Manual extension is fragile
for msg in requests.receive(visibility_timeout=300):
    result = process_complex_task(msg.body)  # May take 10+ minutes
    # If we forget to extend, message expires and redelivers!
    msg.acknowledge()
```

**LeaseExtender solves this by extending only when work is happening:**

- Tool execution beats the heartbeat
- Heartbeat triggers lease extension
- If worker stalls (no beats), lease expires naturally
- Another worker picks up the message

#### How LeaseExtender Works

LeaseExtender attaches to the `Heartbeat` used during message processing. When tools execute and call `heartbeat.beat()`, the LeaseExtender checks if the configured interval has elapsed and extends the message visibility.
...
sequenceDiagram
    ...
    ML->>Tool: execute(context with heartbeat)
    Tool->>HB: beat()
    HB->>LE: on_beat()
...
**Key design principle:** Lease extension is proof-of-work. If the worker is actively processing (beating), the lease extends. If the worker is stuck (no beats), the lease expires naturally and the message becomes visible for another worker to process.

#### Configuration

Configure LeaseExtender via `MainLoopConfig`:
...
config = MainLoopConfig(
    lease_extender=LeaseExtenderConfig(
        interval=60.0,   # Extend at most once per minute
        extension=300,   # Request 5 more minutes on each extension
        enabled=True,    # Enable automatic extension (default)
    ),
)
...
loop.run(visibility_timeout=300)  # Initial 5-minute timeout
```
...
| Parameter | Default | Purpose |
|-----------|---------|---------|
| `interval` | `60.0` | Minimum seconds between extensions (rate limiting) |
| `extension` | `300` | Visibility timeout to request on each extension (seconds) |
| `enabled` | `True` | Whether to enable automatic extension |

**Default rationale:**

- `interval=60`: Rate-limits extensions to once per minute even if tools beat more frequently (avoids excessive API calls)
- `extension=300`: Requests 5 more minutes on each extension, matching the typical `visibility_timeout`
- `enabled=True`: Automatic extension is the safe default for production

#### Heartbeat Propagation

For lease extension to work, the heartbeat must flow from MainLoop through the adapter to tool execution:

```python
# MainLoop passes heartbeat to adapter
...
# Adapter creates ToolContext with heartbeat
context = ToolContext(
    prompt=prompt,
    session=session,
    heartbeat=heartbeat,
...
# Tool handler beats during execution
def my_tool_handler(params: Params, *, context: ToolContext) -> ToolResult:
    # Beat before potentially slow I/O
    context.beat()

    result = expensive_operation(params)

    # Beat after completion
    context.beat()

    return ToolResult.ok(result)
```

**Automatic beating:** Adapters beat automatically at key points (start of tool execution, end of tool execution, LLM call boundaries), so most tools don't need to call `context.beat()` explicitly. Add manual beats only for long-running operations within a single tool.

#### Example: Distributed Code Review

A code review agent processes pull requests from a queue. Each review may take 5-15 minutes (reading files, running linters, analyzing diffs, generating reports):
...
# Configure MainLoop with automatic lease extension
config = MainLoopConfig(
    lease_extender=LeaseExtenderConfig(
        interval=60,     # Extend every minute
        extension=600,   # Request 10 more minutes
    ),
)

class CodeReviewLoop(MainLoop[PRRequest, ReviewReport]):
    def prepare(self, request: PRRequest) -> tuple[Prompt, Session]:
        session = Session(dispatcher=InProcessDispatcher())
        prompt = create_code_review_prompt(request.pr_url)
...
# Start processing PR review requests
loop = CodeReviewLoop(
    adapter=ClaudeAgentSDKAdapter(),
    requests=RedisMailbox(queue_name="pr-reviews"),
    config=config,
)

# Each PR review:
# 1. Receives message (5-minute initial visibility)
# 2. Attaches LeaseExtender to heartbeat
# 3. Tool execution beats heartbeat
# 4. LeaseExtender extends visibility every minute
# 5. If worker stalls, lease expires and message redelivers
# 6. On completion, detaches and acknowledges
loop.run(visibility_timeout=300)
```

As the agent reads files, runs tools, and generates the review, each tool execution beats the heartbeat. Every minute (due to `interval=60`), LeaseExtender extends the visibility by 10 more minutes, preventing premature redelivery.

#### Stuck Worker Behavior

If the worker becomes stuck (infinite loop, deadlock, resource exhaustion), it stops beating. LeaseExtender stops extending, and the visibility timeout expires naturally:

```
Time  | Worker State         | Heartbeat | Visibility
------|----------------------|-----------|------------
0:00  | Processing starts    | Beat      | 5:00 expiry
1:00  | Tool execution       | Beat      | 11:00 (extended)
2:00  | Tool execution       | Beat      | 12:00 (extended)
3:00  | STUCK - infinite loop| No beat   | 12:00 (no extension)
...
12:00 | Still stuck          | No beat   | EXPIRED
12:00 | Message visible again|           | Redelivered to healthy worker
```

This is the correct behavior: stuck workers should not indefinitely hold messages.

#### EvalLoop Integration

`EvalLoop` faces the same lease expiration challenges—evaluation datasets can take significant time to process. EvalLoop includes identical LeaseExtender support:

```python
from weakincentives.runtime import EvalLoop, EvalLoopConfig, LeaseExtenderConfig
...
eval_loop = EvalLoop(
    loop=main_loop,
    requests=RedisMailbox(queue_name="evaluations"),
    config=eval_config,
)

eval_loop.run(visibility_timeout=300)
```

When processing an evaluation dataset:

1. EvalLoop receives the evaluation message
2. Attaches LeaseExtender to its heartbeat
3. Passes heartbeat to `MainLoop.execute()` for each sample
4. Tool execution in MainLoop beats EvalLoop's heartbeat
5. LeaseExtender extends the evaluation message's lease
6. If evaluation stalls, lease expires correctly

This ensures long-running evaluation datasets don't time out prematurely, while stuck evaluations correctly allow the message to be reprocessed.

#### Claude Agent SDK Native Tools

The Claude Agent SDK executes native tools (Bash, Read, Write) in a subprocess managed by the SDK. These tools don't flow through WINK's `ToolExecutor`, so heartbeats are triggered via the hook system:
...
**Limitation:** Native tools beat only at hook boundaries (before/after), not during execution. For a 10-minute Bash build, heartbeats only occur before and after the entire execution.

**Mitigation:** Ensure `extension` is large enough to cover long-running native tools:

```python
config = MainLoopConfig(
    lease_extender=LeaseExtenderConfig(
        interval=60,
        extension=900,   # 15 minutes to cover long Bash commands
    ),
)
```
...
LeaseExtender handles errors gracefully:
...
**Design rationale:** Extension is a reliability optimization, not a correctness requirement. If extension fails, the message may become visible and be processed again—the system handles this via idempotent processing.

#### Disabling Lease Extension

For debugging or testing, disable automatic extension:
...
Or pass `LeaseExtenderConfig(enabled=False)` to selectively disable for specific loops.

#### Comparison: Manual vs. Automatic Extension

| Aspect | Manual Extension | LeaseExtender |
|--------|------------------|---------------|
| Extension logic | Developer responsibility | Automatic |
| Stuck worker behavior | Keeps extending (incorrect!) | Lease expires (correct!) |
| Code complexity | Scattered extension calls | Configured once |
| Proof-of-work | None (time-based) | Heartbeat-based |
| Error-proneness | Easy to forget | Automatic and safe |

**Recommendation:** Use LeaseExtender for all production MainLoop and EvalLoop instances. Manual extension is appropriate only for simple, short-lived tasks with predictable durations.

#### Testing

Test lease extension behavior with mock mailboxes:

```python
from weakincentives.runtime.mailbox.in_memory import InMemoryMailbox

def test_lease_extends_during_tool_execution():
    ...
    # Verify extensions happened
    assert mailbox.extension_count > 0
```

#### Best Practices

1. **Enable by default:** Use `LeaseExtenderConfig` with defaults for all production loops
2. **Set extension > tool duration:** Ensure `extension` covers the longest expected tool execution
3. **Use reasonable intervals:** Default `interval=60` balances API call overhead with timely extension
4. **Beat in long-running tools:** Add `context.beat()` calls in tools that perform extended I/O or computation
5. **Monitor extension failures:** Track `ReceiptHandleExpiredError` warnings to identify lease expiration issues
6. **Test with realistic durations:** Use integration tests with actual tool execution times
