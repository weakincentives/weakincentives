...
- **Backend implementations** - InMemory, SQS, and Redis mailboxes
...
flowchart TB
    ...
    subgraph Queues["Durable Message Queues"]
        RequestQ["Requests Queue<br/>(Redis/SQS)"]
        ResultsQ["Results Queue<br/>(Redis/SQS)"]
    end
...
2. **Durable queues** - Messages survive process restarts and crashes
3. **Competing consumers** - Multiple workers consume from the same queue for parallelism
4. **Dynamic routing** - Reply-to pattern enables flexible result delivery
5. **At-least-once delivery** - Messages are guaranteed to be delivered, potentially multiple times
...
2. **receive()** - Message becomes **Invisible** for `visibility_timeout` seconds
3. **acknowledge()** - Message becomes **Deleted**, permanently removed
4. **timeout expires** - Message returns to **Pending** for redelivery
5. **nack()** - Message returns to **Pending** (optionally after delay)
...
WINK provides three mailbox backends for different deployment scenarios:
...
**Use for:** AWS deployments, managed infrastructure, multi-region
...
mailbox: SQSMailbox[str, None] = SQSMailbox(
    name="my-queue",
    sqs_client=sqs,
...
- ⚠️ Best-effort ordering (use FIFO queues for strict ordering)
- ⚠️ Eventual consistency for count
- ⚠️ Max visibility timeout: 12 hours
...
| Feature | InMemoryMailbox | SQSMailbox | RedisMailbox |
|---------|----------------|------------|--------------|
| **Ordering** | Strict FIFO | Best-effort (Standard)<br/>Strict (FIFO) | Strict FIFO |
| **Long poll max** | Unlimited | 20 seconds | Unlimited |
| **Visibility max** | Unlimited | 12 hours | Unlimited |
| **Count accuracy** | Exact | ~1 min lag | Exact |
| **Durability** | None | Replicated | Configurable |
| **Infrastructure** | None | Managed | Self-hosted |
| **Cost** | Free | Per-request pricing | Redis hosting |
...
3. Raise `MailboxResolutionError` if neither succeeds
...
2. Each sample includes `reply_to=eval_results`
3. Multiple workers consume from `eval_requests` in parallel
4. Each worker processes a subset of samples
5. All workers send results to the same `eval_results` mailbox
6. Client collects all results from single mailbox
...
2. Visibility timeout expires during processing
3. Worker calls `nack()`
...
2. **Receipt Handle Freshness (INV-2):** Each delivery generates a unique receipt handle
3. **Stale Handle Rejection (INV-3):** Operations with old handles fail
4. **Delivery Count Monotonicity (INV-4):** Delivery count never decreases
5. **No Message Loss (INV-5):** Messages are never lost during transitions
6. **Visibility Timeout Correctness (INV-6):** Expired messages eventually return to pending
7. **Handle Uniqueness (INV-7):** Each delivery gets a unique handle
8. **Pending No Duplicates (INV-8):** Pending queue contains no duplicate IDs
9. **Data Integrity (INV-9):** Every queued message has associated data
...
2. **Cross-queue operations not atomic:** Moving messages between queues on different shards is not atomic
3. **Replication lag:** During failover, recently written data may be lost if not yet replicated
...
2. Worker not acknowledging before timeout
3. Worker crash before acknowledge
...
2. Redis reaper hasn't moved expired messages back to pending yet
...
2. Replication lag during failover
3. `min-replicas-to-write` not configured
...
2. Workers failing and nacking messages
3. Messages exceeding delivery limit and not being removed
...
2. **Reply-to pattern** - Dynamic response routing without coupling
3. **Mailbox resolution** - Service discovery for distributed backends
...
- **Backend flexibility** - InMemory, SQS, and Redis implementations
