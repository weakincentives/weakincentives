...
class TestRedisMailboxStandalone:
    ...
    def test_send_returns_message_id(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(name="test", client=client)
            try:
    ...
    def test_acknowledge_expired_handle_raises(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-ack-expired", client=client
            )
            try:
                mailbox.send("hello")
                messages = mailbox.receive(max_messages=1)
    ...
    def test_nack_expired_handle_raises(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-nack-expired", client=client
            )
            try:
                mailbox.send("hello")
                messages = mailbox.receive(max_messages=1)
    ...
    def test_extend_visibility_expired_handle_raises(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-extend-expired", client=client
            )
            try:
                mailbox.send("hello")
                messages = mailbox.receive(max_messages=1)
    ...
    def test_send_serializes_reply_to_name_with_default_resolver(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-reply-to", client=client
            )
    ...
    def test_long_poll_wait_time(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-longpoll", client=client
            )
            try:
    ...
    def test_thread_safety(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[int, None] = RedisMailbox(
                name="test-threadsafe", client=client
            )
            try:
                num_messages = 100
                num_threads = 4
    ...
    def test_message_enqueued_at(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-enqueued", client=client
            )
            try:
                mailbox.send("hello")
                messages = mailbox.receive(max_messages=1)
    ...
    def test_implements_mailbox_protocol(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-protocol", client=client
            )
            try:
    ...
    def test_closed_initially_false(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-closed", client=client
            )
            try:
    ...
    def test_closed_after_close(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-closed-after", client=client
            )
            mailbox.close()
    ...
    def test_receive_returns_empty_when_closed(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox: RedisMailbox[str, None] = RedisMailbox(
                name="test-closed-receive", client=client
            )
            mailbox.send("test")
            mailbox.close()
    ...
    def test_multiple_queues_isolated(self) -> None:
        ...
        with redis_standalone() as client:
            mailbox1: RedisMailbox[str, None] = RedisMailbox(
                name="queue1", client=client
            )
            mailbox2: RedisMailbox[str, None] = RedisMailbox(
                name="queue2", client=client
            )
            try:
...
class TestRedisMailboxCluster:
    ...
    def test_cluster_hash_tags_work(self) -> None:
        ...
        with redis_cluster() as client:
            ...
            mailbox: RedisMailbox[int, None] = RedisMailbox(
                name="test-hashtag", client=client
            )
            try:
                for i in range(10):
                    mailbox.send(i)
    ...
    def test_cluster_purge(self) -> None:
        ...
        with redis_cluster() as client:
            mailbox: RedisMailbox[int, None] = RedisMailbox(
                name="test-cluster-purge", client=client
            )
            try:
                for i in range(5):
                    mailbox.send(i)
