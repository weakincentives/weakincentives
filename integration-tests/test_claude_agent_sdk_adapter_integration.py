...
@dataclass(slots=True)
class TaskParams:
    """Top-level task parameters for progressive disclosure tests."""
...
@dataclass(slots=True)
class ToolsReferenceParams:
    """Parameters for the tools reference section (level 2 - leaf)."""
...
def _build_two_level_hierarchy_prompt(
    ...
    tools_section = MarkdownSection[ToolsReferenceParams](
        ...
        tools=(verify_tool,),
        default_params=ToolsReferenceParams(),
    )
    ...
    instructions_section = MarkdownSection[TaskParams](
        title="Instructions",
        template=(
            "Your task: ${task}\n\n"
...
def test_claude_agent_sdk_adapter_progressive_disclosure_open_sections(
    claude_model: str,
) -> None:
    """Test that Claude Agent SDK adapter works with progressive disclosure.

    This test validates that:
    1. The adapter correctly handles prompts with SectionVisibility.SUMMARY sections
    2. When the model calls open_sections, VisibilityExpansionRequired is raised
    3. After updating visibility_overrides and re-evaluating, sections expand
    4. The model can access and use tools on expanded leaf sections
    """
    verify_tool = _build_verify_tool()
    prompt_template = _build_two_level_hierarchy_prompt(verify_tool)
    params = TaskParams(task="Verify the value 'integration-test-value'")
    ...
    while expansion_count < max_expansions:
        try:
            ...
            # Model requested section expansion via open_sections tool
            expansion_count += 1
    ...
    assert final_response is not None, (
        f"Expected a final response after {max_expansions} expansion iterations."
    )
