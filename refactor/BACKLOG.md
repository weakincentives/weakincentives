...
| 2026-06-03 | Naming: dropped the separate `*Spec` suffix — `*Config` is the single declarative-input idiom; `SandboxSpec` → `SandboxConfig` (since renamed `WorkspaceConfig`); the SDK's isolation `SandboxConfig` is renamed `IsolationConfig` and folded into provider config at M3. |
