...
"""Code review agent demonstrating WINK's prompt composition and MainLoop patterns.

This example shows:
- Prompt templates with typed sections and structured output
- MainLoop-based request processing with mailbox queues
- Progressive disclosure (sections starting summarized)
- Workspace tooling (VFS, Podman, or Claude Agent SDK)
- Auto-optimization of workspace digest on first request

Run with: OPENAI_API_KEY=sk-... uv run python code_reviewer_example.py
"""
...
# Mount configuration for sunfish repository
...
# Allowed domains for Claude Agent SDK network access
...
class ReviewGuidance:
    ...
    focus: str = field(
        default=(
            "Identify potential issues, risks, and follow-up questions for the "
            "changes under review."
        ),
        metadata={"description": "Framing instructions for the review assistant."},
    )


@dataclass(slots=True, frozen=True)
class ReviewTurnParams:
    """Per-turn parameters from user input."""
...
def build_task_prompt(
    *,
    ...
    """Build the code review prompt template.

    Args:
        session: Session for state management.
        use_podman: Use Podman sandbox instead of VFS.
        use_claude_agent: Use Claude Agent SDK workspace.
        workspace_section: Pre-created workspace (Claude Agent mode only).
    """
    _ensure_test_repository_available()

    if use_claude_agent:
        sections = _build_claude_agent_sections(session, workspace_section)
    else:
        sections = _build_standard_sections(session, use_podman)
...
def _build_standard_sections(
    session: Session, use_podman: bool
) -> tuple[MarkdownSection[SupportsDataclass], ...]:
    """Build sections for VFS/Podman mode."""
    workspace_sections = _build_workspace_sections(session, use_podman)
    return (
        ...
        *workspace_sections,
        _build_request_section(),
    )


def _build_claude_agent_sections(
    session: Session, workspace_section: ClaudeAgentWorkspaceSection | None
) -> tuple[MarkdownSection[SupportsDataclass], ...]:
    """Build sections for Claude Agent SDK mode."""
...
def _build_review_guidance_section() -> MarkdownSection[ReviewGuidance]:
    """Build the review brief for VFS/Podman mode."""
...
def _build_claude_agent_guidance_section() -> MarkdownSection[ReviewGuidance]:
    """Build the review brief for Claude Agent SDK mode."""
    return MarkdownSection[ReviewGuidance](
        title="Code Review Brief",
        template=textwrap.dedent("""
...
def _build_reference_section() -> MarkdownSection[ReferenceParams]:
    """Build the progressive disclosure reference section."""
...
def _build_request_section() -> MarkdownSection[ReviewTurnParams]:
    """Build the review request section."""
    return MarkdownSection[ReviewTurnParams](
...
def _build_workspace_sections(
    ...
    """Build workspace sections (VFS or Podman)."""
    ...
    if use_podman:
        ...
        _LOGGER.warning(
            "Podman requested but connection unavailable; falling back to VFS + Asteval."
        )
...
def _sunfish_mounts() -> tuple[HostMount, ...]:
    """Create mount configuration for the sunfish repository."""
    return (
        HostMount(
            host_path="sunfish",
...
def _ensure_test_repository_available() -> None:
    """Verify the test repository exists."""
...
class CodeReviewLoop(MainLoop[ReviewTurnParams, ReviewResponse]):
    ...
    Maintains a persistent session across requests. Runs workspace digest
    optimization automatically on first use.
    ...
    def initialize(
        ...
        """Initialize prompt and session for the request."""
    ...
    def _run_optimization(self) -> None:
        ...
        optimizer = WorkspaceDigestOptimizer(
            context, store_scope=PersistenceScope.SESSION
        )
        prompt = Prompt(
            self._template,
            overrides_store=self._overrides_store,
...
class CodeReviewApp:
    """Interactive REPL for code review using a background MainLoop worker.

    The main thread handles user input while a background thread processes
    requests through the mailbox-based MainLoop.
    """
    ...
    def _run_worker(self) -> None:
        """Background worker processing requests."""
    ...
    def _repl_loop(self) -> None:
        ...
        while True:
            ...
            self._process_request(user_prompt)

    def _process_request(self, user_prompt: str) -> None:
        """Send a request and wait for response."""
        request = ReviewTurnParams(request=user_prompt)
        request_event = MainLoopRequest(request=request, deadline=_default_deadline())
    ...
    def _wait_for_response(
        ...
        """Poll for a response matching our request ID."""
    ...
    def _render_result(self, result: MainLoopResult[ReviewResponse]) -> None:
        ...
        if result.success and result.output is not None:
            print(_format_response(result.output))
        else:
...
def _default_deadline() -> Deadline:
    """Create a fresh default deadline."""
    return Deadline(
        expires_at=datetime.now(UTC) + timedelta(minutes=DEFAULT_DEADLINE_MINUTES)
    )


def _format_response(output: ReviewResponse) -> str:
    """Format a ReviewResponse for display."""
...
def _build_intro(
    ...
    """Build the startup intro banner."""
    if use_claude_agent:
...
def build_claude_agent_adapter() -> tuple[
    ...
    Returns a tuple of (adapter, workspace_section). The workspace section
    should be cloned with the real session before use in prompts.
    """
...
def main() -> None:
    """Entry point for the code reviewer example."""
    args = parse_args()
    configure_logging()
