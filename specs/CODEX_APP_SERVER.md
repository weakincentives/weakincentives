...
(mounts, allowed roots, byte budgets — see `specs/SANDBOX.md`). The
adapter's sandbox provider (default `LocalSandboxProvider`) materializes
it per evaluation; `thread/start` runs with `cwd = sandbox.root`, the
workspace preview is rendered from the opened sandbox, and the sandbox is
closed when evaluation completes. A failed bridged tool rolls back session
