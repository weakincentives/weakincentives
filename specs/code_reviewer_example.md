...
- `build_task_prompt` — main review prompt (namespace
  `examples/code-review:code-review-session`) featuring the shared
  `WorkspaceDigest` section.
- Optimization prompts are assembled inside the adapter by cloning the workspace
  and digest sections; the user-facing prompt remains unchanged.
- Prompts use `MarkdownSection` instances plus the shared planning/workspace
...
- `optimize` refreshes the workspace digest override using the global
...
- `tests/test_code_reviewer_example.py` covers:
  ...
  - Default empty workspace digest placeholder plus override/session persistence.
  - The optimize command path using a stub adapter that emits canned digests.
...
- Extend the workspace digest section to include structured subsections
