...
1. **Registration Lifecycle** – how sections declare tools and how prompts validate and
   expose them.
2. **Context Injection** – the immutable metadata bundle threaded into every handler
   invocation.
3. **Failure Semantics** – the uniform success/failure contract that handlers and
   adapters honor.
...
2. Parameter and result dataclasses reuse existing placeholder validation rules—required
   fields must be supplied when rendering.
3. Declaration order is cached so callers can retrieve tools without re-traversing the
   tree.
...
1. The orchestrator renders (or prepares to render) the prompt and determines the active
   tool call.
2. Immediately before invoking the handler it builds a `ToolContext`, supplying the
   prompt, rendered prompt (if available), adapter, session, and event bus.
3. The handler executes with the `context` keyword argument.
4. After invocation the context instance is discarded; no references are reused across
   tool calls.
