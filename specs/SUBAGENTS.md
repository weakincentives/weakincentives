...
1. **Reuse the parent context**: execute every child with `context.session` and `context.event_bus`. Sharing the parent session and bus keeps telemetry contiguous and avoids fragmenting state across clones. When `context.session` is `None`, continue dispatching with a `None` session but still rely on the parent bus. Because the tool fans out work across threads, both the session and bus implementations MUST be safe for concurrent use.
