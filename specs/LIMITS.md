...
- Extend `ProviderAdapter.evaluate` with two new keyword-only arguments:
  ...
  - Define `TokenBudget` in `weakincentives.limits` as a frozen dataclass with
    ...
    @dataclass(frozen=True)
    class TokenBudget:
        total: ProviderTokenLimit | None = None
        input: ProviderTokenLimit | None = None
        output: ProviderTokenLimit | None = None
        provider_shares: Mapping[str, ProviderTokenLimit] | None = None
    ```

    Each `ProviderTokenLimit` describes ceilings for aggregated tokens, input
    tokens only, and output tokens only. Values MUST be positive integers when
    supplied. `TokenBudget.total` constrains combined input + output usage,
    `TokenBudget.input` constrains all request-side tokens, and
    `TokenBudget.output` constrains all response-side tokens. When
    `provider_shares` is provided, it specifies per-provider limits keyed by
    adapter slug (e.g., `"openai"`) using the same structure. Any omitted field
    (for example `input=None`) indicates that dimension is unbounded while the
    other populated fields remain active.
  ...
  - Callers may omit either argument; adapters validate the arguments eagerly and
    ...
    conflicting limits (for example `total` smaller than the minimum of `input`
    and `output`).
...
- **Token Accounting** – Maintain a `TokenLedger` component that records tokens
  ...
  orchestrator can enforce the corresponding limits precisely.
