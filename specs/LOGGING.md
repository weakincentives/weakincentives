...
The Claude Agent SDK adapter includes a log aggregator that automatically discovers
and tails log files created in the ephemeral `.claude` directory during prompt
evaluation. This provides visibility into Claude Code's internal operations.
...
"log", "transcript", or "debug". Files in `settings.json` and `skills/` are excluded
as they are generated by WINK, not Claude Code.

**Content Tailing**: Once discovered, log files are tailed for new content. Each
non-empty line is emitted as a separate `log_aggregator.log_line` event with the
file name and content. File positions are tracked to emit only new content on
subsequent polls.
