...
## Event-Based Collection (Alternative)

For detailed audit trails, emit discrete events instead of aggregating:

```python
@FrozenDataclass()
class AdapterCallEvent:
    """Discrete event for each adapter call."""
...
# Store in append-only slice
session[AdapterCallEvent].append(event)

# Query recent events
recent = session[AdapterCallEvent].where(
    lambda e: e.created_at > datetime.now(UTC) - timedelta(hours=1)
)
```

This approach trades storage efficiency for complete auditability. Use for
debugging or when detailed per-request analysis is needed.
