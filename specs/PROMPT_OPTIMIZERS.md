...
Prompt optimizers adjust prompts after adapter evaluation so future executions
benefit from discovered improvements without manual edits. This document
formalizes the shared contract every optimizer must follow in order to integrate
with the runtime and the prompt override system.
...
- **Session**: Event-sourced execution trace composed of the
  `PromptRendered`, `ToolInvoked`, and `PromptExecuted` events defined in
  `specs/EVENTS.md`. Sessions are constructed through the reducer rules outlined
  in `specs/SESSIONS.md`.
- **Trajectory**: A multi-render execution record defined in
  `specs/TRAJECTORIES.md`. Each trajectory bundles the reducer snapshots
  captured after every evaluation for a single adapter run.
...
1. The optimizer receives a batch of trajectories plus access to the override
   store.
1. All trajectories in the batch MUST originate from prompts that resolve to the
   same `PromptDescriptor`. Diverging descriptors are a planning error and MUST
   be detected by re-deriving descriptors from the snapshots.
1. The optimizer produces an immutable plan describing intended store
   mutations. Planning MUST NOT mutate trajectories, snapshots, or the store.
...
1. Optimizers SHOULD produce identical plans when re-run over the same batch.
   ...
   nondeterministic behavior and, when possible, expose controls (e.g.,
   explicit seeds or reproducibility toggles) that callers can use to regain
   determinism.
...
class Trajectory(NamedTuple):
    ...
    # demand using these descriptors; trajectories never persist prompt text.
...
- `plan` inspects one or more trajectories and produces an `OverridePlan`
  scoped to a single override tag. Empty batches MUST raise. Each trajectory
...
- `descriptor`: The `PromptDescriptor` derived from the canonical prompt in the
  trajectories at planning time. Plans MUST persist this descriptor so apply
...
all trajectories map back to the same descriptor; mismatches MUST raise.

## Validation Requirements

Before applying overrides, the optimizer (or caller) MUST:

1. Recompute the `PromptDescriptor` for each trajectory under consideration by
...
- Planning SHOULD draw conclusions from the `PromptRendered`, `ToolInvoked`, and
  `PromptExecuted` event slices held in each trajectory snapshot sequence.
